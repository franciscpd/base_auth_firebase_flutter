...
import 'package:ezsale/core/common/md5.dart';
...
class GravatarProvider extends PhotoUrlProvider {
  ...
  Future<PhotoUrlInfo> emailToPhotoUrl(
    String email, {int size = 100, bool checkIfImageExists = false}
  ) async {
    if (email != null) {
      var hash = md5(email.trim().toLowerCase());

      if (checkIfImageExists) {
        var url = 'https://www.gravatar.com/avatar/$hash?d=404&s=${size.toString()}&r=g';
