...
import 'package:ezsale/core/widgets/screen_logo.dart';
import 'package:ezsale/core/widgets/tablet_aware_scaffold.dart';
...
import 'package:ezsale/core/auth/handlers/email/sign_up_button.dart' as email;
...
class SplashState extends ProgressActionableState<Splash> {
  ...
  Widget _handleSnapshot(
    AppModel model, BuildContext context, AsyncSnapshot<LoginState> snapshot
  ) {
    if (snapshot.hasData) {
      ...
      if (_loginLoader == null) {
        _loginLoader = FutureBuilder<LoginState>(
          ...
          builder: (_, AsyncSnapshot<LoginState> snapshot) =>
            _handleSnapshot(model, context, snapshot),
        );
      }
  ...
  Widget _buttons(
    AppInfo appInfo, AuthService authService, {String errorMessage}
  ) {
  ...
  Widget _buildTabletView(
    AppInfo appInfo, Widget loader, Color splashBackgroundColor, Color splashForegroundColor
  ) {
    return Row(
      children: <Widget>[
        Expanded(
