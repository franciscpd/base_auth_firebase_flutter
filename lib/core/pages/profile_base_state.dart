...
import 'package:ezsale/core/common/dialog.dart';
import 'package:ezsale/core/dialogs/show_info_dialog.dart';
import 'package:ezsale/core/dialogs/show_error_dialog.dart';
import 'package:ezsale/core/dialogs/show_ok_cancel_dialog.dart';
import 'package:ezsale/core/widgets/email_image_circle_avatar.dart';
import 'package:ezsale/core/auth/handlers/email/change/email_page.dart';
import 'package:ezsale/core/auth/handlers/email/change/password_page.dart';
import 'package:ezsale/core/auth/handlers/user/change/display_name_page.dart';
import 'package:ezsale/core/auth/handlers/linkAccounts/link_account_page.dart';
import 'package:ezsale/core/auth/handlers/user/closeAccount/close_account_page.dart';
...
abstract class ProfileBaseState<T extends StatefulWidget> extends State<T> {
  ...
  Future _sendVerificationEmail(AuthService authService, AuthUser userInfo) async {
    var passwordProvider = _getPasswordProvider(authService);

    if (passwordProvider == null) {
      return await showErrorDialog(context, 'Email/Password provider is not configured');
    } else {
      try {
  ...
  Future _acknowledgeEmailSend() async {
    return await showInfoDialog(
      ...
      context: context,
      caption: 'Email sent',
      message: 'Pleae check your email.',
    );
  }
  ...
  Future _sendVerifyEmail(AuthService authService, AuthUser userInfo) async {
    return await showOkCancelDialog(
      ...
      message: 'Would you like to send a verification email to ${userInfo.email}',
    );
  }
